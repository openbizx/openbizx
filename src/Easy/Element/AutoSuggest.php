...
class AutoSuggest extends OptionElement
{
    ...
    public function render ()
    {
        if (defined('OPENBIZ_JSLIB_BASE') && OPENBIZ_JSLIB_BASE == 'JQUERY') {
			...
			$sHTML .= "<script>Openbizx.AutoSuggest.init('$this->formName','AutoSuggest','$inputName');</script>";
			return $sHTML;
		}
        ...
        if (strpbrk($first_half, ':')) {
            ...
            $sHTML .= "<script>Openbizx.AutoSuggest.init('$this->formName','AutoSuggest','$hInputName','$inputChoice');</script>";
            ...
            $sHTML .= "<div id=\"$inputChoice\" class=\"autocomplete\" style=\"display:none\"></div>\n";
            $sHTML .= "<script>Openbizx.AutoSuggest.init('$this->formName','AutoSuggest','$inputName','$inputChoice');</script>";
        }
        return $sHTML;
    }
